...
/*   Updated: 2021/04/24 22:20:03 by jfrancis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		get_integer(va_list args, t_specs *spec)
{
	...
	if (n >= 0 && spec->prec_size >= 0 && spec->precision == 1 && (spec->lalign == 1 || spec->prec_size > spec->width))
	{
		spec->filler = '0';
	...
	{
		spec->filler = '0';
		if (spec->width - spec->prec_size == n_len)
			output = spec->width - spec->prec_size;
		else
			output = spec->width - spec->prec_size - 1;
		print_fill(output, spec);
	}
	if (n == 0 && spec->width > spec->prec_size && spec->lalign == 1)
	{
		spec->filler = ' ';
		if (spec->prec_size > 0 && spec->prec_size < n_len)
			output = spec->width - n_len;
		else if (spec->prec_size == 0)
			output = spec->width;
		else
			output = spec->prec_size - n_len;
		print_fill(output, spec);
